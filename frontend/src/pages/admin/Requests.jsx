  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 p-4">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm dark:shadow-gray-900/50 p-6 mb-6">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
            <div>
              <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Employee Requests</h1>
              <p className="text-gray-600 dark:text-gray-400 mt-1">Review and manage employee leave, OD, and work from home requests</p>
            </div>
          </div>
          ...
          {stats && (
            <div className="grid grid-cols-1 sm:grid-cols-4 gap-4 mt-6">
              <div className="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-lg">
                <div className="text-2xl font-bold text-blue-600 dark:text-blue-400">{stats.statusStats?.total || 0}</div>
                <div className="text-sm text-blue-600 dark:text-blue-400">Total Requests</div>
              </div>
              <div className="bg-yellow-50 dark:bg-yellow-900/20 p-4 rounded-lg">
                <div className="text-2xl font-bold text-yellow-600 dark:text-yellow-400">{stats.statusStats?.pending || 0}</div>
                <div className="text-sm text-yellow-600 dark:text-yellow-400">Pending</div>
              </div>
              <div className="bg-green-50 dark:bg-green-900/20 p-4 rounded-lg">
                <div className="text-2xl font-bold text-green-600 dark:text-green-400">{stats.statusStats?.approved || 0}</div>
                <div className="text-sm text-green-600 dark:text-green-400">Approved</div>
              </div>
              <div className="bg-red-50 dark:bg-red-900/20 p-4 rounded-lg">
                <div className="text-2xl font-bold text-red-600 dark:text-red-400">{stats.statusStats?.rejected || 0}</div>
                    ...
                    <div className="flex-1">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-3">
                        <div>
                          ...
                          <div className="text-sm text-gray-600 dark:text-gray-400">
                            {request.employeeId ? `${request.employeeId.name} (${request.employeeId.employeeId})` : 'Employee not found'}
                          </div>
                          <div className="text-xs text-gray-500 dark:text-gray-500">
                            {request.employeeId?.department || 'Department not available'}
                          </div>
                        </div>
                        
                        <div>
                          <div className="text-sm font-medium text-gray-700 dark:text-gray-300">Period</div>
                          <div className="text-sm text-gray-600 dark:text-gray-400">
                      ...
                      {request.approvedBy && (
                        <div className="text-xs text-gray-500 dark:text-gray-500 mt-2">
                          {request.status === 'approved' ? 'Approved' : 'Rejected'} by {request.approvedBy?.name || 'Unknown'} on {format(new Date(request.approvedAt), 'MMM dd, yyyy HH:mm')}
                        </div>
                      )}
                    </div>
            ...
            <div className="px-6 py-4 border-t border-gray-200 dark:border-gray-700">
              <div className="flex items-center justify-between">
                ...
                <div className="flex gap-2">
                  <button
                    onClick={() => fetchRequests({ ...filters, page: pagination.currentPage - 1 })}
      ...
      <ConfirmationModal
        ...
        onClose={() => setActionModal({ show: false, request: null, action: '', comments: '' })}
        onConfirm={handleActionSubmit}
        title={`${actionModal.action === 'approved' ? 'Approve' : 'Reject'} Request`}
        message={
          <div className="space-y-4">
            <p>
              Are you sure you want to {actionModal.action === 'approved' ? 'approve' : 'reject'} this request?
            </p>
            <div>
